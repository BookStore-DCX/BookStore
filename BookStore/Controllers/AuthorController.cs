...
public class AuthorController : ControllerBase
{
    ...
    public AuthorController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        return Ok(
            ApiResponse<IEnumerable<AuthorDto>>.Ok(
    ...
    public async Task<IActionResult> Create([FromBody] AuthorCreateDto dto)
    {
        ...
        int nextId = authors.Any()
            ? authors.Max(a => a.AuthorId) + 1
            : 1;
        ...
        return CreatedAtAction(
            nameof(GetByName),
            new { authorName = author.LastName },
            ApiResponse<AuthorDto>.Created(
                _mapper.Map<AuthorDto>(author)
            )
        );
    }
