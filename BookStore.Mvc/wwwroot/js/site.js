document.addEventListener("DOMContentLoaded", () => {
    ...
    const setupCardPager = (pager) => {
        const targetSelector = pager.getAttribute("data-card-pager");
        if (!targetSelector) return;
        ...
        const renderPage = () => {
            const size = getPageSize();
            maxPage = Math.max(0, Math.ceil(items.length / size) - 1);
            pageIndex = Math.min(pageIndex, maxPage);
